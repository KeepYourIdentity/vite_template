...
export default function Skeleton({ variant, className = "", customBackground = "" }: SkeletonState): ReactElement {
  const safeMerge =
    `relative overflow-hidden bg-zinc-300 ${variant !== void 0 && variants[variant]} ${className.trim()}`.trim();
  const safeBackground =
    `absolute inset-0 animate-shimmer ${customBackground || "bg-linear-to-r from-transparent via-white/30 to-transparent"}`.trim();
  return (
